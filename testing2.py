...
def generateReport2(): #Will generate the report for tags
    fullText2 = readtxt(filename=filepath4,
                       color=(255, 0, 0))
    #filtered_L = readtxt(filename=filepath2, #For future use
    #                   color=(255, 0, 0))
    fullText10 = str(fullText2)
    s = ''.join(fullText10)
    w = (s.replace (']', ']\n\n'))
    paragraph = report3.add_paragraph()
    filepath5 = str(filepath4.rsplit('/', 1)[-1]) # change filepath to something.docx
    filepath5 = filepath5.split('.', 1)[0] # removes .docx of the file name
    print(filepath5 + " report created")
    runner = paragraph.add_run("\n" + "Document Name: " + filepath5 + "\n")
    runner.bold = True #makes the header bold
    # w will be used in the future
    w = (w.replace ('([', ''))
    w = (w.replace (',', ''))
